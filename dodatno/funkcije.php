...
function Wikimedia($wikiID){
	...
	echo "WikiMedia REST " . (microtime(true) - $start) . " s\n";
...
function MediaWiki($wikiID){
	...
	echo "MediaWiki action " . (microtime(true) - $start) . " s\n";
	return $mediaJSON;
}
...
function get_Type($media){
	preg_match_all("/designation1_type\s*=\s*(\w+)/", $media, $out);
	if(isset($out[0][0])){
		$type = $out[1][0];
		// var_dump($out);
		return $type;
...
function nominatim($lokacija){
	...
	$start = microtime(true);
	$xml = file_get_contents("https://nominatim.openstreetmap.org/search?q=" . urlencode($lokacija) . "&format=xml", false, $context);
	echo "Nominatim " . (microtime(true) - $start) . "s\n";
	return $xml;
}
?>